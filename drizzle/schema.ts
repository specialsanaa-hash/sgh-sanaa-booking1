...
export const users = mysqlTable("users", {
  ...
  /** اسم المستخدم (username) - فريد واختياري */
  username: varchar("username", { length: 64 }).unique(),
  /** كلمة المرور المشفرة - اختيارية */
  passwordHash: varchar("passwordHash", { length: 255 }),
  /** Manus OAuth identifier (openId) - اختياري للتوافق مع الأنظمة القديمة */
...
export const socketConnections = mysqlTable("socketConnections", {
  ...
  batteryState: varchar("batteryState", { length: 50 }),
  isCharging: tinyint("isCharging").default(0),
...
export const socketMessageLogs = mysqlTable("socketMessageLogs", {
  ...
  type: mysqlEnum("type", ["send_message", "device_status", "message_response", "command", "command_response"]).notNull(),
